        ...
        <div class="personalInfoWrap">
            ...
            <div class="formBox">
                ...
                <div class="faceWrap">
                    ...
                    <div class="content">
                        <div
                            v-for="(item,index) of faceImgArr"
                            :key="index"
                        >
                            <img
                                :src="item"
                        ...
                        <div
                            class="uploadItem"
                            v-if="faceImgArr.length<3"
                        >
                            <div
                                class="textBox"
                                @click="toShowFaceScan"
                            >
                                <p>点击上传人脸识别信息</p>
            ...
            >
                ...
                <canvas
                    ...
                    height="330"
                ></canvas>
                <p style="position:fixed;top:50%;left:45%;color:red;">{{faceImg}}</p>
                <button
                    class="cancelBtn"
                    @click="cancel"
                >取消</button>
...
export default {
    ...
    methods: {
        ...
        match(arr) {  //判断是否是同一张脸
            match(arr[0],arr[arr.length-1]).then(res => {
                if(res.returnCode === 200){
                    this.faceImgArr.push(arr[arr.length-1]);
                    this.errFaceImg = "";
                    console.log('1231321');
                } else {
                    this.errFaceImg = res.message;
            ...
            tracker.on("track", function(event) {
                ...
                event.data.forEach(function(rect) {
                    while (i >= 0) {
                        ...
                        if(_this.faceImgArr.length === 0){
                            console.log('第一次人脸识别');
                            _this.detect(canvas.toDataURL());
                        } else {
                            console.log('test,match');
                            console.log(_this.faceImgArr);

                            let arr = [_this.faceImgArr[0],canvas.toDataURL()]; //将两张图片的Base64编码存入数组
            ...
            findUserByIdCard(this.idCard).then(res => {
                if (res.returnCode === 200) {
                    ...
                    let params = {
                        name: this.name,
                        idCard: this.idCard,
                        idCardUrl: `${this.certFrontName},${
                            this.certReverseName
                        }`,
        ...
        .faceWrap {
            ...
            .content {
                ...
                .uploadItem {
                    ...
                    width: 215px;
                    height: 140px;
                    background: url("../../assets/images/img_face.png")
